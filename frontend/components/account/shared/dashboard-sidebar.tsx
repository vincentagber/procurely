    ...
    <aside className="w-[280px] flex-shrink-0 bg-[#0A1140] text-white hidden lg:flex flex-col sticky top-20 h-[calc(100vh-80px)] overflow-y-auto scrollbar-hide shadow-2xl z-10 pb-10">
      <div className="p-8 border-b border-white/10">
        <Link href="/" className="group block">
          <h2 className="text-2xl font-black tracking-tight flex items-center gap-1 group-hover:text-blue-400 transition-colors">
            Procurely<span className="text-[10px]">&trade;</span>
          </h2>
        </Link>
      </div>
      ...
      <div className="p-8 border-b border-white/10 flex items-center gap-4 text-left">
        ...
        <div className="min-w-0">
          <p className="font-bold text-[14px] leading-tight text-white truncate">{user?.fullName || "User Account"}</p>
          <p className="text-[10px] text-white/50 font-bold uppercase tracking-wider mt-1 truncate">{user?.role || "Procurement Manager"}</p>
        </div>
      </div>
