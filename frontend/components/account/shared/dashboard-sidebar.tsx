  ...
  const isLinkActive = (href: string) => {
   ...
   const sidebarContent = (
    <div className="flex flex-col h-full bg-[#0C1457] pt-[17.47px] pr-[8.73px]">
      {/* Brand & User Card */}
      <div className="shrink-0 mb-8 pl-6">
      ...
      <div className="flex-1 overflow-y-auto scrollbar-hide">
        <nav className="flex flex-col" style={{ gap: '13.1px' }}>
          {menuItems.map((item) => {
            ...
            return (
              <Link
                key={item.href}
                href={item.href}
                className={`w-full flex items-center gap-3 pl-6 pr-4 py-2.5 transition-all h-[44px] ${
                  active
          ...
          <button
            onClick={logout}
            className="w-full flex items-center gap-3 pl-6 pr-4 py-2.5 transition-all h-[44px] bg-transparent text-white hover:bg-white/10 font-bold mt-auto"
          >
            <LogOut size={18} />
